...
//
const uint16_t DOUBLECLICKTIME(600);  // Count two clicks as doubleclick if both received within this time
const uint16_t PRESSTIME(700);
const uint16_t HOLDTIME(1500);        // Report held button after time
...
private:
  ...
  uint16_t buff;        // Moving window to record multiple readings
  uint8_t  dbnceIntvl;  // How long to lockout bounce AFTER press/release
  bool     buttonDown;  // Raw data. We don't need to see it, we don't want to see it.
  ButtonState state[2];
...
public:
  // Constructor
  MagicButton(uint8_t pin, bool pullup):  // Active LOW if pullup == TRUE
    pin(pin),
    pullup(pullup)
  {
    buff       = 0;
